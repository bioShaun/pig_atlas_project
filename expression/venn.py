...
def plot_venn(gene_list_file, out_dir):
    gene_list = []
    gene_list_files = gene_list_file.split(',')
    for each_f in gene_list_files:
        diff_df = pd.read_table(each_f, header=None, index_col=0)
        gene_list.append(diff_df.index)
    combination_list = [PurePath(each).stem for each in gene_list_files]
    comb_num = len(gene_list)
